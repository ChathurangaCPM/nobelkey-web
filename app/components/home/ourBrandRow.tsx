...
const OurBrandRow: React.FC<OurBrandRowProps> = ({
    ...
    const rowItems: LinkItems[] = typeof linkCards === 'string' ? JSON.parse(linkCards) : (linkCards || []);
    return (
                ...
                <div className="pb-5 lg:p-20 lg:border-r-[1px] lg:border-black/10 flex-1">
                    <div className="w-[100%] flex flex-col gap-4">
                        {title && <div className="relative pl-4">
                            ...
                            <h1 className="text-[16px] lg:max-w-[50%] font-headingFontExtraBold leading-[25px] font-extrabold">{title}</h1>
                        </div>}
                        {description && <p className="text-sm lg:max-w-[65%]">{description}</p>}
                    </div>
                </div>

                <div className="flex-1 flex flex-col lg:flex-row justify-center gap-5">
                    {rowItems && rowItems?.map((d, index) => (
                        <Link key={index} href={d?.link || '#'} className="lg:p-10 flex-1 flex items-center flex-col justify-center gap-5 lg:border-r-[1px] lg:border-black/10 group">
                            <Image src={d?.imageUrl || 'https://dummyimage.com/600x250/ddd/fff'} alt={''} className="w-full" width={300} height={200} />
